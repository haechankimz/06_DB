...
--> 하동운, 이오리가 누락됨
--> 왜? DEPT_CODE(NULL) 값과 같은 값이 DEPT_ID 컬럼에 존재하지 않아서
...
FROM EMPLOYEE , DEPARTMENT 
WHERE DEPT_CODE = DEPT_ID(+);
-- DEPT_CODE의 값이 DEPT_ID와 일치하지 않아도 결과에 포함 시켜라
...
--> 모든 경우의 수를 보고 싶을 때 빼고는 직접 작성하는 경우가 없음
	--> 자연 조인 실패 결과로 교차 조인 결과가 출력됨
		--(교차 조인 결과가 보이면 -> 자연 조인 잘못썼네)
...
-- EMPLOYEE.JDB_CODE (CHAR(2))
-- JOB.JOB_CODE			 (CHAR(2))
	--> 컬럼명, 자료형이 모두 일치 == NUTURAL JOIN의 대상
...
SELECT EMP_NAME, JOB_NAME, SALARY , 
	CASE 
	WHEN BONUS IS NOT NULL THEN	(SALARY * 12) * (BONUS + 1) 
