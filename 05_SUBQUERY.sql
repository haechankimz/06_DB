...
WHERE SALARY > (
	...
	WHERE JOB_CODE = MAIN.JOB_CODE	/*메인한테 받은 행의 값*/
	-- MAIN한테 전달 받은 JOB_CODE 직급의 평균 급여 조회
);


/* 1) 메인 쿼리 한 행의 값을 서브쿼리로 전달
 * 2) 서브 쿼리에서 전달받은 값을 이용해서 SELECT 수행
...
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음'), DEPT_CODE, 
			 JOB_NAME, HIRE_DATE 
FROM EMPLOYEE "M"
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
JOIN JOB USING (JOB_CODE)
WHERE HIRE_DATE = (
	/*메인 쿼리에서 전달받은 행의 컬럼 값 중
	 * DEPT_CODE 값을 이용해 해당 부서에서 가장 빠른 입사일 조회*/
	SELECT MIN(HIRE_DATE) 
	FROM EMPLOYEE "S"
	WHERE NVL(S.DEPT_CODE, '소속없음') = NVL(M.DEPT_CODE, '소속없음') 
				--> NULL은 비교가 불가능하기 때문에 
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, (SALARY * 12) * (BONUS +1)
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE 
