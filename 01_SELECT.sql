...
-- SELECT문 작성 시
-- 컬럼명에 산술 연산을 직접 작성하면 조회결과인
-- RESULT SET에 연산 결과가 반영되어 조회된다
...
-- SYSDATE			: 현재 시간(년,월,일,시,분,초)데이터를 저장
-- SYSTIMESTAMP : 현재 시간(년,월,일,시,분,초,ms + 지역(local)) 조
...
-- CEIL(숫자) : 올림 처리
-- 현재 시간 - 생년월일
SELECT CEIL((SYSDATE - TO_DATE('2002-02-04', 'YYYY-MM-DD'))/ 365) 
FROM DUAL;
...
SELECT 
	CEIL((SYSDATE - TO_DATE('2002-02-04', 'YYYY-MM-DD'))/ 365) AS 나이
FROM DUAL;
...
-- 지정된 컬럼의 값이 중복되는 경우 이를 한 번만 표시할 때 사용
...
/* FROM절에 지정된 테이블에서 
 * WHERE로 행을 먼저 추려내고 추려진 결과 행들 중에서
 * SELECT절에 원하는 컬럼만 조회 */


-- EMPLOYEE 테이블에서 연봉이 5000만원을 이하인 사원의
-- 사번, 이름, 연봉 조회
...
ORDER BY DEPT_CODE; -- ASC 생략 가능(기본 값이 오름차순)
...
-- ** 정렬 시 SELECT절에 작성된 컬럼을 그대로 따라 적는 경우가 많음
...
-- 오름차순 + NULLS FIRST(NULL인 경우 제일 위에 위치)
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
ORDER BY PHONE NULLS FIRST;

-- 오름차순 + NULLS LAST(NULL인 경우 제일 아래에 위치)
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
ORDER BY PHONE NULLS LAST; -- NULLS LAST 생략가능(기본값)


-- 내림차순 + NULLS FIRST(NULL인 경우 제일 위에 위치)
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
ORDER BY PHONE DESC NULLS FIRST; 
--정렬 기분 -> NULL 위치 순서대로 해석
