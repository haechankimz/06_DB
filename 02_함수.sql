...
-- 단, 이메일 길이가 12 이하인 행만 
-- 이메일 길이 오름차순 조회 
SELECT EMP_ID, EMAIL, LENGTH(EMAIL) "이메일 길이"
FROM EMPLOYEE
...
-- 문자열에서 5번 부터 검색 시작해서 두 번째로 찾은 'B'의 위치 조회
...
-- 문자열 공백 제거
SELECT '   기  준   ',
		TRIM(LEADING  ' ' FROM '   기  준   '), -- 앞 쪽 공백 제거
		TRIM(TRAILING ' ' FROM '   기  준   '), -- 뒤 쪽 공백 제거
		TRIM(BOTH     ' ' FROM '   기  준   ') -- 양 쪽 공백 제거
FROM DUAL;
...
SELECT MONTHS_BETWEEN('2024-04-28', SYSDATE)
FROM DUAL;
...
SELECT EMP_ID, EMP_NAME, HIRE_DATE,
	CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE) /12) ||'년차' "N년 차"
FROM EMPLOYEE;

-- MONTHS_BETWEEN()은 윤년(2월 29일 포함된 해) 계산이 자동으로 수행
--> YEAR, MONTH 단위 계산 시 더 정확한 값을 얻어낼 수 있다


-- ADD-MONTHS(날짜, 숫자) : 날짜를 숫자만큼의 개월 수를 더하여 반환
SELECT SYSDATE, -- 2/27 
	SYSDATE + 29,  --3/27
	SYSDATE + 29 + 31,  --4/27
...
-- 다음 달 첫 번째, 마지막 날짜
SELECT LAST_DAY(SYSDATE) + 1, -- 첫 번쨰  
...
-- EMPLOYEE 테이블에서
-- 이름, 급여, 성과급(급여 * BONUS) 조회
-- 단, 성과급이 없으면 0 으로 표시
SELECT EMP_NAME, SALARY, NVL(SALARY * BONUS, 0) 성과급 
FROM EMPLOYEE;
...
SELECT EMP_ID, EMP_NAME, 
	NVL2(PHONE, RPAD(SUBSTR(PHONE, 1, 3), LENGTH(PHONE), '*') , '없음') 전화번호
FROM EMPLOYEE;
...
/*2*/ SELECT SUM(SALARY) 			-- 3) SALARY 컬럼 값의 합을 조회
